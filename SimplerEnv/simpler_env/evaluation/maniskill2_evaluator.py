...
def run_maniskill2_eval_single_episode(
    ...
    is_src_obj_grasped = episode_stats.get("is_src_obj_grasped", False)
    is_src_obj_moved = episode_stats.get("moved_correct_obj", False)
    ...
    if policy_model != 'lapa':
        ckpt_path_basename = ckpt_path_basename.split("/")[-1]
    else: 
        ckpt_path_basename = ckpt_path_basename.split("/")[-2] + '_' + ckpt_path_basename.split("/")[-1]
    if ckpt_path_basename == None or ckpt_path_basename == "None":
        ckpt_path_basename = policy_model
    ...
    return success == "success", is_src_obj_grasped, is_src_obj_moved
...
def maniskill2_evaluator(model, args):
                ...
                if args.obj_variation_mode == "xy":
                    for obj_init_x in args.obj_init_xs:
                        for obj_init_y in args.obj_init_ys:
                            final_success, grasp_success, move_success = run_maniskill2_eval_single_episode(
                                    obj_init_x=obj_init_x,
                                    obj_init_y=obj_init_y,
                                    **kwargs,
                                )
                            success_arr.append(final_success)
                            grasp_arr.append(grasp_success)
                            moved_arr.append(move_success)
    ...
    return success_arr, grasp_arr, moved_arr
