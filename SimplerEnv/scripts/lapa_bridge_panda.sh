...
ckpt_path="params::/workspace/LAPA/lapa_checkpoints/params"
...
CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference_lapa.py --policy-model ${policy_model} --ckpt-path ${ckpt_path} \
    ...
    --load-llama-config 7b --update-llama-config "dict(action_vocab_size=245,delta_vocab_size=8,sample_mode='text',theta=50000000,max_sequence_length=32768,scan_attention=False,scan_query_chunk_size=128,scan_key_chunk_size=128,scan_mlp=False,scan_mlp_chunk_size=8192,scan_layers=True)" \
