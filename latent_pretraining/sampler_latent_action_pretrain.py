...
from latent_pretraining.delta_llama_action import VideoLLaMAConfig, FlaxVideoLLaMAForCausalLM 
    ...
    def _process_frame(self, images):
        if self.FLAGS.image_aug:
            preprocessor_finetune = albumentations.Compose([
                    ...
                    CenterCropThenResize(height=256, width=256, scale=[0.9, 0.9], ratio=[1.0, 1.0],interpolation=cv2.INTER_LINEAR)
                ])
        else:
            preprocessor_finetune = albumentations.Compose([
                    albumentations.LongestMaxSize(max_size=256),  # Resize the longest side to 256
